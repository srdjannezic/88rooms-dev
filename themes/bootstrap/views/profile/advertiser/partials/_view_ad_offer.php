            ...
            <?php if (!$data->checkIfAproved($data->ad_id)) { ?>
                <div class="span-mini-box rightAlign right">
                    <div class="btn-group" id="control-btns">
                        <?php echo CHtml::link("<i class='icon-ok'></i> " . __('Izaberi ponudu'), Yii::app()->createUrl("profile/profileOfferApprove", array("id" => $data->id)), array("class" => "btn", 'confirm' => 'Da li ste sigurni da ste odabrali pravu ponudu?')); ?>                    
                    </div>
                </div>
            <?php } ?>
        </div>
