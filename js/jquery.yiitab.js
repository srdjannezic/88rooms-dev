	...
	$.extend($.fn, {
		yiitab: function() {
			...
			function activate(id) {
                if($(id).parent().find('>ul a[href="'+id+'"]').hasClass("disabled")) {
                    return false;
                }
				...
				$container.find('>ul a').parent().removeClass('active');
				$container.find('>ul a[href="'+id+'"]').parent().addClass('active');
				$container.children('div').hide();
                //$tab.removeClass("offscreen");
				$tab.show();
			}
            ...
            activate(this.find('>ul a').attr("href"));
