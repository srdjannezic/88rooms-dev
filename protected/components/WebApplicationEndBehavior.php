...
/**
 * Description of WebApplicationEndBehavior
 *
 * @author Nikola
 */
class WebApplicationEndBehavior extends CBehavior {
    ...
    protected function changeModulePaths($event) {
        ...
        if ($event->sender->theme !== null)
            $event->sender->viewPath = $event->sender->theme->basePath.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR;
        else
            $event->sender->viewPath .= DIRECTORY_SEPARATOR.$this->_endName;
        //Utility::dump($event->sender->viewPath);
    }
