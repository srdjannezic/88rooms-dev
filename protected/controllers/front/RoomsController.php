...
class RoomsController extends Controller {
    ...
    public function actionIndex($slug) {
        ...
        $this->render("templates/$model->template->short_code", array(
            'model' => $model,
        ));
    }
    ...
    public function actionView($slug) {
        $model = $this->loadModel($slug, TRUE);
        $description = (!empty($model->meta_description) ? $model->meta_description : $model->description);
        $title = (!empty($model->meta_title) ? $model->meta_title : $model->name);
        ...
        Yii::app()->clientScript->registerMetaTag($this->createAbsoluteUrl("rooms/view", array("slug" => $model->slug)), null, null, array('property' => "og:url"));
    ...
    public function loadModel($slug, $ml = false) {
        $model = HotelRoom::model()->findByAttributes(array("slug" => $slug));
