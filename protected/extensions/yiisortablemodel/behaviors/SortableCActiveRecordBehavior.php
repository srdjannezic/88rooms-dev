...
class SortableCActiveRecordBehavior extends CActiveRecordBehavior {
    ...
    public function beforeSave($event) {
        ...
        if ($sender->isNewRecord) {
            ...
            if ($this->relation_field) {
                $condition['condition'] = "{$this->relation_field}=:val";
                $condition['params'] = array(":val" => $sender->{$this->relation_field});
            }
            //Utility::dump($condition);
            $last_record = $model->find($condition);
            if ($last_record) {
    ...
    public function afterDelete($event) {
        ...
        $following_records = $model->findAll($condition);
        foreach ($following_records as $record) {
            if ($record->{$this->orderField} >= $sender->{$this->orderField}) {
