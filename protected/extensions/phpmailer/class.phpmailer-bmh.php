...
class BounceMailHandler {
  ...
  function processBounce($pos,$type,$totalFetched) {
    ...
    if ($type == 'DSN') {
      ...
      if(isset($structure->parts) && count($structure->parts)) {
          ...
          foreach($structure->parts as $part) {
            $body_part= imap_fetchbody($this->_mailbox_link, $pos, $n);
            
            $matches= array();
            preg_match("/X-log-id: ([0-9]+)/", $body_part, $matches);
            if(count($matches)==2) {
                $this->x_log_id= $matches[1];
            }
    ...
    if ($rule_no == '0000') { // internal error      return false;
      ...
      $params = array($pos,$bounce_type,$email,$subject,$xheader,$remove,$rule_no,$rule_cat,$totalFetched, $this->x_log_id);
